...
template xdr(T)
{
    ...
    void encode(XdrDataOutputStream stream, ref const T from)
    {
        T.encode(stream, from);
    }
    void encode(XdrDataOutputStream stream, ref T[] from)
    {
        int size = cast(int)from.length;
        stream.writeInt(size);
        for (int i = 0; i < size; i++)
        {
            T.encode(stream, from[i]);
        }
    }
}
